...
- **El `<slug>` del nombre de fichero se genera una sola vez** (al crear el evento, combinando fecha + nombre) **y nunca se regenera**, ni siquiera si luego se edita la fecha del evento. Es deliberado: el `id` público (`<feed url>/events/<slug>`) se construye a partir del slug y está documentado como "nunca cambiar tras publicar" — los consumidores lo usan para actualizar el evento en vez de duplicarlo, así que renombrar el fichero al cambiar la fecha rompería esa garantía. Coste asumido: el nombre de fichero puede quedar desalineado con la fecha real tras una edición (p. ej. `2026-07-...json` para un evento movido a agosto). El nombre de fichero es mnemotécnico, no dato — la fecha que importa es la del campo `startDate` dentro del JSON.
