...
// The OTE Spec 0.1.0 schemas, from the @opentechevents/schema package
...
export const eventSchema: AnySchemaObject = {
  ...
  "$id": "https://opentechevents.org/schema/v0.1/event.schema.json",
  "title": "OTE Event",
  ...
  "$defs": {
    "event": {
      ...
      "properties": {
        ...
        "license": {
          "description": "License of THIS DATA, not of the event. SPDX identifier (CC0-1.0, CC-BY-4.0…) or a URL.",
          "$ref": "#/$defs/license",
          "examples": [
            "CC-BY-4.0",
            "CC0-1.0"
        ...
        "status": {
          "description": "A cancelled or postponed event MUST stay published: removing it leaves a dead event in subscribers' calendars.",
          "enum": [
            "scheduled",
            "cancelled",
            "postponed",
    ...
    "instant": {
      "description": "An absolute point in time, WITH offset or Z. Used for metadata (when data was fetched), never for when an event happens.",
      "type": "string",
    ...
    "location": {
      ...
      "properties": {
        "venue": {
          "description": "Human-readable physical location. Its presence means the event has a physical venue.",
          "type": "string",
          "minLength": 1,
          "examples": [
            "El Cable, Almería"
...
export const feedSchema: AnySchemaObject = {
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://opentechevents.org/schema/v0.1/feed.schema.json",
  "title": "OTE Feed",
  ...
  "properties": {
    ...
    "license": {
      "description": "License for the feed's contents. Acts as the DEFAULT for every event that does not declare its own. SPDX identifier or URL.",
      "$ref": "https://opentechevents.org/schema/v0.1/event.schema.json#/$defs/license",
      "examples": [
        "CC-BY-4.0",
        "CC0-1.0"
      ]
    ...
    "updatedAt": {
      ...
      "$ref": "https://opentechevents.org/schema/v0.1/event.schema.json#/$defs/instant",
      "examples": [
    ...
    "events": {
      ...
      "type": "array",
      "items": {
        "$ref": "https://opentechevents.org/schema/v0.1/event.schema.json#/$defs/event"
      }
    }
  }
};
...
/**
 * OTE Spec 0.1.0 has no recommended (quality) profile: it was introduced
